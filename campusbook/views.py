...
def index(request):
    ...
    if announcement:
        ...
        announcement_body = "Welcome to CampusBook"
        
    posts = Post.objects.filter(user__in=list(buddies)).order_by('-date_created')[:100]
    form = PostForm()
    context = {'index_link_active': "link-active", 'announcement': announcement_body, 'posts': posts, 'form': form}
    return render(request, 'campusbook/index.html', context)
...
def privacy(request):
    """Privacy Policy page"""
    return render(request, 'campusbook/privacy.html')

def terms(request):
    """Terms & Conditions page"""
    return render(request, 'campusbook/terms.html')

def about(request):
    """About us page"""
    return render(request, 'campusbook/about.html')

def contact(request):
    """Contact us page (with message functionality)"""
    return render(request, 'campusbook/contact.html')

def logout(request):
    if request.user.is_authenticated:
        django_logout(request)
        return redirect('campusbook:login')
    else:
        raise Http404    
